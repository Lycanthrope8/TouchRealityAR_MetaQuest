...
namespace ARObjectDetection.Gateway
{
    public class SkillGatewayClient : MonoBehaviour
    {
        ...
        private SkillInterpretResponse ParseInterpretResponse(string body)
        {
            ...
            // 4. Re-read requires_confirmation directly from the raw JSON body.
            //    JsonUtility maps field names by exact string match, so it correctly
            //    handles snake_case "requires_confirmation" but silently returns false
            //    if the server ever sends camelCase "requiresConfirmation". Reading
            //    it manually here covers both casings and makes the mapping explicit.
            resp.requires_confirmation =
                      ExtractBoolField(body, "requires_confirmation") ||
                      ExtractBoolField(body, "requiresConfirmation");

            if (config != null && config.enableDebugLogs)
                Debug.Log($"[SkillGatewayClient] parsed: decision_id={resp.decision_id} " +
                          $"requires_confirmation={resp.requires_confirmation} " +
                          $"decisionType={resp.decision?.decisionType}");
        ...
        /// <summary>
        /// Read a top-level boolean field from a JSON string.
        /// Returns true only when the key is present and its value is exactly "true".
        /// Used to handle requires_confirmation which may arrive as snake_case or
        /// camelCase depending on the gateway version, without relying on JsonUtility's
        /// exact-match field binding.
        /// </summary>
        private static bool ExtractBoolField(string json, string key)
        {
            ...
            // Match the literal token "true" (case-insensitive for robustness)
            return i + 4 <= json.Length &&
                   string.Compare(json, i, "true", 0, 4, StringComparison.OrdinalIgnoreCase) == 0;
        }
